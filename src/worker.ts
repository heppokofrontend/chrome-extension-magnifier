...
const contextMenuIdReset = `${contextMenuId}-reset`;
...
const remove = () => {
  ...
  chrome.contextMenus.remove(contextMenuIdReset, () => chrome.runtime.lastError);
    ...
    ({ menuItemId }: chrome.contextMenus.OnClickData, tab) => {
      if (tab?.url?.startsWith('http') && typeof tab.id === 'number') {
        switch (menuItemId) {
          ...
          case contextMenuIdReset:
            chrome.tabs.sendMessage(tab.id, { taskId: 'reset' }).catch(console.log);
            break;
